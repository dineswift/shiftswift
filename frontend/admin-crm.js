...
(async function initAdminCrm() {
  ...
  async function loadPipeline(q = "") {
    const query = q.trim();
    const path = query ? `/admin/crm/pipeline?q=${encodeURIComponent(query)}` : "/admin/crm/pipeline";
  ...
  async function openEntityDrawer(type, id, silent = false) {
    ...
    if (els.entityDrawerMeta) {
      const bits =
        type === "account"
          ? [record.email, record.phone, record.website].filter(Boolean)
          : [record.account_name, record.job_title, record.email, record.phone].filter(Boolean);
  ...
  els.pipelineSearch?.addEventListener("input", () => {
    debouncedSearch("pipeline", loadPipeline, els.pipelineSearch);
  });
