...
(function initShiftSwiftUnifiedLogin() {
  ...
  function showMfaStep(username, meta = {}) {
    ...
    const passkeyAvailableFlag =
      typeof meta === "object" && meta
        ? Boolean(meta.passkeyAvailable ?? meta.passkey_available)
        : Boolean(meta);
    ...
    const passkeyAvailable = featureOn && passkeyAvailableFlag;
    const loginShell = document.getElementById("login-shell");
    const mfaPanel = document.getElementById("mfa-panel");
    const enrollmentPanel = document.getElementById("mfa-enrollment-panel");
  ...
  async function startMfaEnrollment(data, redirectUrl) {
    ...
    const lead = enrollmentPanel?.querySelector(".portal-login-card-lead");
    if (lead) {
      lead.textContent =
        "Recommended for account security. Scan the QR code in Google Authenticator, Authy, or Microsoft Authenticator, then enter a 6-digit code. You can set this up now or skip and enable it later in settings.";
    }
    if (enrollmentPanel && !document.getElementById("mfa-enrollment-skip")) {
      const submitBtn = document.getElementById("mfa-enrollment-submit");
      const skipBtn = document.createElement("button");
      skipBtn.type = "button";
      skipBtn.className = "btn portal-login-submit portal-login-submit--secondary";
      skipBtn.id = "mfa-enrollment-skip";
