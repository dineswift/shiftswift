(function () {
  ...
  function renderMetrics() {
    ...
    const cards = [
      {
        ...
        sub: "Estimated ex-VAT",
        valueClass: "master-metric__value--accent",
      },
      { label: "Total tenants", value: stats.total_tenants ?? "0", sub: "Registered businesses" },
      {
        label: "Active (paying)",
        value: stats.active_paying ?? "0",
        sub: `${stats.conversion_rate_pct ?? 0}% conversion`,
        valueStyle: "color: var(--master-green)",
      },
      {
        label: "Trialing",
        value: stats.trialing ?? "0",
        sub: stats.avg_trial_days_remaining != null ? `Avg ${stats.avg_trial_days_remaining} days left` : "",
      },
    ...
    els.metrics.innerHTML = cards
      .map((card) => {
        ...
        const sub = card.sub ? `<div class="master-metric__sub">${card.sub}</div>` : "";
        return `<article class="master-metric"><div class="master-metric__label">${card.label}</div><div class="master-metric__value${valueCls}"${valueStyle}>${card.value}</div>${sub}</article>`;
      })
      .join("");
  }

  function initMasterIcons() {
    document.querySelectorAll("[data-nav-icon]").forEach((el) => {
      const name = el.dataset.navIcon;
      if (window.AdminIcons?.svg && name) {
        el.innerHTML = window.AdminIcons.svg(name);
      }
    });
    document.querySelectorAll("[data-btn-icon]").forEach((el) => {
      const name = el.dataset.btnIcon;
      if (window.AdminIcons?.svg && name) {
        el.innerHTML = window.AdminIcons.svg(name);
      }
    });
  ...
  function selectTenantForBulk(id) {
    if (state.selectedId === id) {
      clearTenantSelection();
      return;
    }
    state.selectedId = id;
    state.selectedTenant = state.tenants.find((row) => row.id === id) || null;
    renderTable();
    renderCards();
    syncDeleteSelectedButton();
  }
  ...
  function bindRowEvents(container) {
    container.querySelectorAll("[data-view]").forEach((btn) => {
      btn.addEventListener("click", (event) => {
        ...
        openTenantWindow(Number(btn.dataset.view));
      });
    });
    container.querySelectorAll("[data-tenant-id]").forEach((el) => {
      el.addEventListener("click", (event) => {
        if (event.target.closest(".master-col-select")) {
          event.stopPropagation();
          selectTenantForBulk(Number(el.dataset.tenantId));
          return;
        }
        if (event.target.closest("[data-view]")) return;
        const id = Number(el.dataset.tenantId);
        if (id) openTenantWindow(id);
      });
    });
  }
      ...
      host.innerHTML = `
        <div class="master-revenue-summary">
          <article class="master-metric master-metric--hero">
            ...
            <div class="master-metric__value master-metric__value--accent">${formatMoney(stats.mrr_gbp)}</div>
            <div class="master-metric__sub">Estimated ex-VAT · ARR ${formatMoney(stats.arr_gbp)}</div>
          </article>
