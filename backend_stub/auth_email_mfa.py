...
from core.notifications import send_email_content, smtp_configured
...
def invalidate_email_mfa_codes(*, conn: Any, username: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
            UPDATE mfa_email_codes
            SET consumed_at = COALESCE(consumed_at, NOW())
            WHERE lower(username) = lower(%s) AND consumed_at IS NULL
            """,
            (username,),
        )
...
def send_login_email_code(
    *,
    ...
    """Deliver the OTP email. Raises RuntimeError when SMTP is unavailable or delivery fails."""
    ...
    if not looks_like_email(username):
        raise RuntimeError("This account does not have an email address for verification codes")

    audience = "employee" if role == "employee" else ("platform" if role == "admin" else "hr")
...
def issue_and_send_email_mfa(
    *,
    ...
    """Create OTP, email it, commit. Respects resend cooldown unless force_resend is unused for first send."""
    ...
    return {
        "email_hint": mask_email(username),
